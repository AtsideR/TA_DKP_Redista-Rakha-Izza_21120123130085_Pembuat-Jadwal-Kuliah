...
class jadwalkuliah:
    ...
    def buat_widgets(self):
        ...
        self.total_jadwal_label = tk.Label(self.master, text="Total Jadwal: 0", bg=self.bg_color, fg=self.fg_color, font=("Arial", 10, "bold"))
        self.total_jadwal_label.grid(row=5, column=6, padx=5, pady=5, sticky="nsew")
    ...
    def tambah_acara(self):
        ...
        if hari and acara and waktu_mulai and waktu_selesai:
            ...
            self.jadwal[hari].sort(key=lambda x: x[0])
            self.perbarui_jadwal()
            self.acara_entry.delete(0, tk.END)
            self.update_total_jadwal()
        else:
    ...
    def update_total_jadwal(self):
        total_jadwal = sum(len(acara_list) for acara_list in self.jadwal.values())
        self.total_jadwal_label.config(text=f"Total Jadwal: {total_jadwal}")
